        ...
        this.setState({ input: event.target.value }, () => { /*set state extendido*/
            ...
            if (event.target.value !== '') {
                ...
                results = this.state.peliculas.filter((unapelicula) => {
                    return unapelicula.title.toLowerCase().includes(event.target.value.toLowerCase())
                ...
                this.setState({ resultadopeliculas: '' })
            }
        });
    }
    render() { //show puede valer 4 cosas: 2 strings o 2 arrays
        ...
        return (
            <React.Fragment>
                ...
                <button className="button-card" onClick={() => this.cargarMas()}>Cargar más películas </button>
            </React.Fragment>
        )
