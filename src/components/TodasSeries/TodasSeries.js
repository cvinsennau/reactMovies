        ...
        this.setState({ input: event.target.value }, () => { /*set state extendido*/
            ...
            if (event.target.value !== '') {
                ...
                results = this.state.series.filter((unaSerie) => {
                    console.log(unaSerie, "unaserie")
                    return unaSerie.original_name.toLowerCase().includes(event.target.value.toLowerCase())
                    ...
                    //que me devuelva con return si el titulo de cada serie incluye lo que escribio el usuario, lo paso a lowercase y me fijo si me trae lo q escribio el usuario event.target.value, si esto es true, se guarda la serie en la variable results 
                })
                console.log(results)
                if (results.length === 0) {
                    show = `No se encontraron resultados de búsqueda`;
                    console.log(show);
                }
